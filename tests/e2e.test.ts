...
/**
 * Windows can briefly hold a temp dir while a child process (tool shell,
 * MCP transport) drains its last handles; under full-suite parallelism an
 * immediate rmSync then fails with EPERM/EBUSY. Retry with a short sleep —
 * the handles always release.
 */
function rmWithRetry(target: string, tries = 5): void {
